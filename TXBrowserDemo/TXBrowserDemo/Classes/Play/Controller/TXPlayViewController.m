...
#import "TXPlayDetailViewController.h"
...
- (void)analysisDatasourcePlist {
    ...
    for (int index = 0; index < datasoucePlist.count; ++ index) {
        ...
        if ([groupName isEqualToString:@"第一分区"]) {
            TXLog(@"第一分区");
            /** 1.第一个分组 **/
...
//    TXItemGroupModel *group1 = [TXItemGroupModel itemGroupModelWithGroupHeaderTitle:@"第二分区" groupFooterTitle:@"" groupMembers:@[collectionItemModel_Double]];
//    
//    /** 第三个分组 **/
...
//    TXItemGroupModel *group2 = [TXItemGroupModel itemGroupModelWithGroupHeaderTitle:@"第三分区" groupFooterTitle:@"" groupMembers:@[collectionItemModel_Triple]];
//    [self.dataSource addObject:group0];
//    [self.dataSource addObject:group1];
...
- (void)pushDetailViewControllerWithTitle:(NSString *)title detailSources:(NSMutableArray *)detailSources {
    TXPlayDetailViewController *detailViewController = [[TXPlayDetailViewController alloc] init];
    detailViewController.title = title;
    detailViewController.dataSource = detailSources;
